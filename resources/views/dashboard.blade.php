    ...
    <div class="p-6 space-y-8">
        ...
        @if (auth()->user()?->isAdmin())
            <div class="bg-yellow-100 text-yellow-800 p-4 rounded shadow text-center mb-6">
                ⚠️ Estás accediendo al panel de usuarios, pero sos administrador. Usá el
                <a href="{{ route('reclamos.admin') }}" class="underline text-blue-600">Panel de Administración</a>.
            </div>
        @endif

        {{-- Botones de acceso a otras funcionalidades --}}
        <div class="flex flex-wrap gap-4 justify-center mb-6">
            <a href="{{ route('reclamos.crear') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                ➕ Crear Reclamo
            </a>
        </div>
